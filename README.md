...
Весь код лежит в `src/`, в корне — только конфигурация. Интерфейс собран
в одной папке, логика разложена по модулям — по смыслу, а не по типу файла:

```
src/
  app/              роутинг Next: страница и API — тонкие, только вызовы модулей
    page.tsx          единственный экран
    api/route/        поиск маршрутов

  frontend/         всё, что видно в браузере
    components/route/   форма, календарь, подсказки городов, карточки, гостиницы
    design.ts           палитра и подписи из макета
    format.ts           цены, время, склонения
    config.ts           примеры, особые условия, тайминги этапов
    globals.css         базовые стили

  modules/          логика, зависимости однонаправленные: agent → routing → tutu
    tutu/             интеграция с MCP Туту
      client.ts         подключение, кэш в Postgres, обёртка инструментов
      types.ts          типы ответов Туту и разбор списков
      config.ts         адрес сервера, время жизни кэша, лимиты для модели
      fixtures/         настоящие ответы Туту для разработки и прогрева кэша
    routing/          домен маршрутов
      builder.ts        ядро: узлы, плечи, стыковки, запасной план с ночёвкой
      hubs.ts           справочник «регион → административный центр»
      hub-suggest.ts    подсказка городов моделью (необязательная)
      cities.json       1134 города России для подсказок при вводе
      config.ts         запас на пересадку, границы перебора
    agent/            агентный цикл (в работе)

  lib/              инфраструктура: клиент Prisma и сгенерированный код

scripts/            утилиты: прогрев кэша, просмотр кэша, поиск из консоли, smoke
prisma/             схема БД: McpCache
```

**Почему внутри модулей нет слоёв** `domain/`, `infra/`, `api/`. Такая нарезка
оправдана на десятках тысяч строк и полутора десятках доменов. Здесь весь код —
около трёх с половиной тысяч строк и один сценарий: слои дали бы папки по одному
файлу и пустой `contract/`. Когда модуль перерастёт тысячу строк, слои появятся
вокруг существующих файлов без переписывания.
