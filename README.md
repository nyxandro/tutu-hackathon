...
Код разложен по модулям — по смыслу, а не по типу файла:
...
components/     интерфейс: форма, календарь, карточки, гостиницы
lib/            общее: конфиг интерфейса, палитра, форматтеры, клиент Prisma
...
Зависимости однонаправленные: `agent` → `routing` → `tutu`. Модуль не знает
о том, кто его использует.
